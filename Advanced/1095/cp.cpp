...
struct node {
	char id[10];
	int time, flag = 0; // in +1, out -1 方便计算
};
...
int main() {
	...
	for(int i = 0; i < n; i++) {
		...
		record[i].flag = strcmp(temp, "in") == 0 ? 1 : -1; // in 1, out -1
	}
	// 获得车辆进出记录
	sort(record.begin(), record.end(), cmp1); // sort by id and time
	// 记录时间。
	map<string, int> mapp;
	for(int i = 0; i < n - 1; i++) {
		// 同一个 id， 车牌一次进出信息
		//  i 进 i +1 出
		// 一队进出的信息将会保存起来
		// 如果连续多次 in，那么就取最后一次 in 和 out 的记录。
		// 如果连续多次 out，那么取第一次 in 和 out 的。总之是 in out 相连。
		// out 必须在 in 之后。
	...
	for(int i = 0; i < car.size(); i++) {
		if(i == 0)
				...
				cnt[i] = cnt[i - 1] + car[i].flag; // 进 +1, out -1
	}
	for(int i = 0; i < k; i++) {
		int h, m, s;
		scanf("%d:%d:%d", &h, &m, &s);
